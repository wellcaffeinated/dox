...
exports.parseComment = function(str) {
  ...
  if (~str.indexOf('\n@')) {
	...
	comment.examples = comment.tags.filter(function(tag){
...
exports.parseTag = function(str) {
	var tag = {}
	  ...
	  , head = body.shift()
	  , m = head.match(/{[^@]+}/)
	  , types = (m && m.length)? m[0] : null
	  , parts = head.trim().replace(types,'').split(/ +/)
	...
	switch (type) {
		...
		case 'return':
			...
			tag.description = [parts.join(' ')].concat(body).join('\n');
			break;
		case 'see': // format: @see {Long title}| <Long title more text lastly> <http://site.com|internal/link>
			var url = parts.pop();
			...
			tag.title = exports.parseTagTypes(types).concat(parts).join(' ');
			if(tag.title == ''){
			  tag.title = url;
			}
			break;
		...
		case 'example':
			tag.types = exports.parseTagTypes(types);
			tag.description = markdown(escape([parts.join(' ')].concat(body).join('\n')));
			break;
		default:
			tag.types = exports.parseTagTypes(types);
			tag.description = [parts.join(' ')].concat(body).join('\n');
